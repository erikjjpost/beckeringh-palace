...
from compiler.component_accessibility import ComponentAccessibilityConstraint
...
from compiler.data_sources import DataSourceConstraint
...
from compiler.wallpaper_families import WallpaperFamilyConstraint
    ...
    def evalueer(self, context: ConstraintContext):
        ...
        for obj in context.objecten:
            if objectsoortdefinitie(obj.soort) is None:
                diagnostics.append(Diagnostic(code="BP3001", boodschap=f"Onbekende objectsoort '{obj.soort}'", locatie=obj.bronlocatie))
            ...
            for veld in verplichte_velden:
                ...
                if not isinstance(waarde, str) or not waarde.strip():
                    diagnostics.append(Diagnostic(
                        code="BP3002",
                        boodschap=f"Native object '{obj.id}' van soort '{obj.soort}' vereist tekstveld '{veld}'",
    ...
    def evalueer(self, context: ConstraintContext):
        ...
        for obj in token_objecten.values():
            ...
            if type_waarde not in {token_type.value for token_type in TokenType}:
                diagnostics.append(Diagnostic(code="BP3101", boodschap=f"Token '{obj.id}' heeft onbekend type '{type_waarde}'", locatie=obj.eigenschaplocaties.get("type", obj.bronlocatie)))
                continue
            if not isinstance(waarde, str) or not waarde.strip():
                diagnostics.append(Diagnostic(code="BP3102", boodschap=f"Token '{obj.id}' vereist tekstveld 'waarde'", locatie=obj.eigenschaplocaties.get("waarde", obj.bronlocatie)))
                continue
            ...
            if referentie is not None:
                ...
                if doel is None:
                    diagnostics.append(Diagnostic(code="BP3103", boodschap=f"Token '{obj.id}' verwijst naar onbekend token '{referentie}'", locatie=obj.eigenschaplocaties.get("waarde", obj.bronlocatie)))
                    continue
                doeltype = doel.eigenschappen.get("type")
                if doeltype != token.type.value:
                    diagnostics.append(Diagnostic(code="BP3104", boodschap=f"Token '{obj.id}' van type '{token.type.value}' verwijst naar token '{referentie}' van type '{doeltype}'", locatie=obj.eigenschaplocaties.get("waarde", obj.bronlocatie)))
            elif not waarde_past_bij_type(token.type, token.waarde):
                diagnostics.append(Diagnostic(code="BP3105", boodschap=f"Waarde '{token.waarde}' past niet bij token-type '{token.type.value}'", locatie=obj.eigenschaplocaties.get("waarde", obj.bronlocatie)))
        return tuple(diagnostics)
