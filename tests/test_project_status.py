...
class ProjectStatusTests(unittest.TestCase):
    ...
    def test_status_is_typed_product_context_for_every_backend(self) -> None:
        ...
        def render(_objecten, product):
            ...
            self.assertEqual(
                "M11.9c", product.project_status.current_milestone.id
            )
            self.assertEqual("M11.9c", product.project_status.next_step.id)
            self.assertEqual(
                "M11.9b", product.project_status.last_completed_milestone.id
            )
            self.assertEqual(
                118, product.project_status.last_completed_milestone.pull_request
            )
