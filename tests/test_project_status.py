...
class ProjectStatusTests(unittest.TestCase):
    ...
    def test_total_progress_is_deterministically_calculated(self) -> None:
        ...
        self.assertEqual(51, calculate_overall_progress(self.status))
    ...
    def test_status_is_typed_product_context_for_every_backend(self) -> None:
        ...
        def render(_objecten, product):
            ...
            self.assertEqual(51, product.project_status.overall_progress)
            self.assertEqual(10, len(product.project_status.areas))
            self.assertEqual(
                "M11.3h", product.project_status.current_milestone.id
            )
            self.assertEqual("M11.3i", product.project_status.next_step.id)
